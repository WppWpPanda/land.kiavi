<?php
/**
 * Companies_Table — Display and Manage Title Companies from Database
 *
 * A child class of Wpp_List_Table that displays title companies
 * with custom actions (Edit, Delete) and full data from the `wpp_companies` table.
 *
 * Features:
 * - Pagination
 ...
 *
 * Displays title companies from the `wpp_companies` database table with Edit and Delete actions.
 *
 ...
 * @since 1.0.0
 */
class Companies_Table extends Wpp_List_Table {
	 ...
	 * @since 1.0.0
	 */
	protected $table_name = 'wpp_companies';
	...
	/**
	 * Whether to enqueue a custom JavaScript file for company-specific logic.
	 *
	 * Set to false to disable loading of `companies-table.js`.
	 *
	 * @var bool
	 * @access protected
	 * @since 1.0.0
	...
	protected $columns = array(
		'id'                      => 'ID',
		'comp_title_company_name' => 'Title Company Name',
		'comp_city'               => 'City',
		'comp_state'              => 'State',
		'comp_phone'              => 'Phone',
	 ...
	 *
	 * @example
	 *     Sorting URL: ?orderby=comp_city&order=ASC
	 */
	...
	public function column_actions( $item ) {
		...
		$delete_url = wp_nonce_url( $delete_url, 'delete_company_' . $item['id'], 'company_nonce' );

		// Create Edit button
		$actions = sprintf(
			'<a href="#" class="button button-small comp-edit" style="font-size:12px;padding:4px 8px;">✏️ Edit</a>'
		);

		// Create Delete button with confirmation
		$actions .= ' ' . sprintf(
				'<a href="%s" class="button button-small comp-delete" style="color:#a00;font-size:12px;padding:4px 8px;" onclick="return confirm(\'Are you sure you want to delete this company?\\nThis action cannot be undone.\')">🗑️ Delete</a>',
				esc_url( $delete_url )
			);
	 ...
	 * @since 1.0.0
	 */
	public function column_comp_title_company_name( $item ) {
		return '<strong>' . esc_html( $item['comp_title_company_name'] ) . '</strong>';
	}

	/**
	 * Enqueue custom JavaScript for company interactions (e.g., modal, AJAX).
	 *
	 * Loads: /wpp-core/wpp_libs/wpp_list_table/js/companies-table.js
	...
	protected function enqueue_custom_js() {
		...
		$js_url = WPP_LOAN_MANAGER_URL . 'assets/js/companies-table.js';
		...
		// wp_localize_script( 'wpp-companies-table-js', 'wppCompaniesConfig', array(
		//     'ajax_url' => admin_url( 'admin-ajax.php' ),
		//     'nonce'    => wp_create_nonce( 'wpp_company_nonce' )
		// ) );
	}
